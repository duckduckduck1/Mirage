...
Документ описывает текущий путь установки Mirage на новый VPS: базовый bootstrap,
панель 3x-ui/Xray, основной VPN-профиль VLESS Reality и production-checkpoint.
Резервные протоколы, домен, публичные subscription-ссылки и Telegram-прокси `mtg`
добавляются отдельными этапами.
...
Клиентское приложение
  └─ подключается к VPN: SERVER_IP:443
...
## Шаг 4. Production-checkpoint VLESS Reality

После ротации секретов закрепи фактическое рабочее состояние: VLESS Reality
работает на `443/tcp`, панель доступна только через SSH-туннель, лишние публичные
порты закрыты.

Выполни runbook:

- [Production-checkpoint VLESS Reality](runbooks/vless-production-checkpoint.md)

Ключевые требования:

- `ufw` открывает только `22/tcp` и `443/tcp`;
- `x-ui` слушает только `127.0.0.1:ПОРТ_ПАНЕЛИ`;
- Xray слушает публичный `443/tcp`;
- `8388/tcp` не слушает и не открыт;
- пароль панели сменён;
- subscription path не равен `/sub/`;
- активный VLESS-клиент ротирован;
- свежий backup сохранён вне git.
...
## Шаг 6. Резервный протокол

Резервный протокол добавляй только отдельным этапом после backup. Shadowsocks-2022
остаётся диагностическим вариантом, но не входит в текущую production-схему:

- [Проверка Shadowsocks-2022](runbooks/setup-shadowsocks-2022.md)

Если резервный протокол не используется, держи его inbound выключенным, а
публичный порт закрытым в `ufw`.

## Шаг 7. Telegram-прокси mtg
...
- [ ] `8388/tcp` закрыт, если резервный протокол не используется.
