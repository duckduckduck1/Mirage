...
## Шаг 3. Домен и подписки

Домен нужен для стабильных ссылок, а не для защиты IP от блокировки.

Рекомендуемая схема DNS:

```text
sub.ДОМЕН   A   SERVER_IP   # subscription-ссылки
vpn.ДОМЕН   A   SERVER_IP   # адрес подключения, опционально
```

Для Cloudflare держи записи в режиме **DNS only**. Проксирование Cloudflare не
заменяет TCP-транспорт для Reality и не скрывает VPS от блокировки IP.

В 3x-ui используй subscription-ссылки для выдачи профилей. Если сервер переезжает,
ты восстанавливаешь конфигурацию на новом VPS и меняешь DNS A-запись. Клиенты
получают новый адрес при следующем обновлении подписки.
...
После рабочей настройки сделай первый бэкап панели и сохрани его вне git. Для
PostgreSQL используй backup/export панели или `pg_dump`; для SQLite сохраняй
`x-ui.db` и связанные конфиги.
...
- [ ] В клиентской ссылке указан публичный IP или домен.
- [ ] Shadowsocks работает на `8388/tcp`.
- [ ] Subscription-ссылка использует домен.
