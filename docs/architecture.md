...
раздел о пределах фильтрации в [документе установки](setup/README.md).
...
- **Infrastructure as Code (поэтапно)** — настройка сервера кодифицируется в
  Ansible, а 3x-ui управляется через `xui-ops`; provisioning через Terraform —
  в будущих версиях (ADR-0006).
...
| xui-ops | API-автоматизация 3x-ui: inbound, клиенты, ссылки, backup | Docker на VPS |
...
- **Reality (VPN).** В TLS-рукопожатии поле **SNI** содержит домен крупного
  публичного TLS-сервиса. В текущем рабочем профиле используется
  `www.amazon.com`; `www.microsoft.com` остаётся резервным кандидатом. Для DPI
  соединение похоже на обычный HTTPS-трафик к выбранному домену. **XTLS-Vision**
  усиливает стойкость к активному зондированию.
- **FakeTLS (Telegram).** mtg будет заворачивать MTProto в TLS под выбранный
  домен маскировки. Домен для MTProxy фиксируется на отдельном этапе.

Подробно про установку и проверки — в [документе установки](setup/README.md).
...
2. **Низкий TTL DNS.** A-записи `vpn.ДОМЕН` и `sub.ДОМЕН` держим с коротким TTL
   (60–300 с), чтобы переключение применялось быстро. `vpn.ДОМЕН` в Cloudflare
   всегда остаётся в режиме **DNS only**: проксировать Reality и произвольный
   TCP-трафик через обычный Cloudflare proxy нельзя.
3. **Бэкап конфигурации панели.** 3x-ui хранит состояние в SQLite (`x-ui.db`) или
   ...
   новом сервере. Процедура закреплена в [runbook бэкапа 3x-ui](runbooks/backup-xui.md).
...
3. Установить 3x-ui/Xray и настроить API-доступ по гайду установки.
4. Восстановить базу 3x-ui из бэкапа → те же клиенты и UUID.
5. Проверить `xui-ops vpn-diagnose` и рабочий VLESS-профиль.
6. Переключить A-записи `vpn.ДОМЕН` и `sub.ДОМЕН` на новый IP.
7. Клиенты при следующем обновлении подписки автоматически переходят на новый
   сервер.
...
администратора по SSH-ключу и настраивает базовую защиту. Второй этап закреплён
через 3x-ui, Docker и `xui-ops`: панель закрыта за SSH-туннелем, VLESS Reality
работает на `443/tcp`, профили `main`, `partner`, `shared` создаются через API.
Следующая цель — кодифицировать установку сервисов в Ansible, затем добавить
MTProxy.
...
- **Ansible** отвечает на вопрос «что на сервере настроено» (идемпотентно). Уже
  покрыт первый слой: доступ, `ufw`, `fail2ban` и SSH-hardening.
- **xui-ops** отвечает за операционное управление 3x-ui: создание inbound,
  синхронизацию клиентов, диагностику, ссылки и backup.
...
- Домен и публичные subscription-ссылки.
- Периодичность бэкапа базы 3x-ui и проверка восстановления на отдельном VPS.
- Автоматизация установки 3x-ui/Xray через Ansible.
- MTProxy/FakeTLS.
