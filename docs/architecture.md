...
|---|---|---|
| VPS (Ubuntu/Debian) | Хост с публичным IP | reseller-VPS, Германия (Франкфурт), ручная выдача |
...
| Ansible | Установка и настройка сервисов (план) | локально по SSH |
...
4. **Воспроизводимая настройка (Ansible).** Сам сервер покупается у ритейлера
   вручную (API нет), но его настройка воспроизводится Ansible'ом за минуты;
   provisioning через Terraform отложен (ADR-0006).

### Сценарий «сервер забанили»

1. Купить новый VPS у ритейлера (вручную) — получить новый IP.
2. `ansible-playbook` — установить и настроить весь стек (Terraform — позже).
...
- **Ansible** отвечает на вопрос «что на сервере настроено» (идемпотентно) и
  сохраняет быструю часть миграции.
